...
bool Texture::ImgLoad(const std::string& FileName) {
	Destroy();
	SDL_Texture* NewSDLTexture = nullptr;

	SDL_Surface* LoadedSurface = IMG_Load(FileName.c_str());
	if (!LoadedSurface) {
		Debug::Log(EMessageType::ERR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to load " + FileName + ". SDL Error: " + SDL_GetError());
	}
	else {
		if (SDL_SetColorKey(LoadedSurface, SDL_TRUE, SDL_MapRGB(LoadedSurface->format, 0, 0xFF, 0xFF)) < 0) {
			Debug::Log(EMessageType::ERR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to set color key for " + FileName + ". SDL Error: " + SDL_GetError());
		}

		NewSDLTexture = SDL_CreateTextureFromSurface(SDLRenderer, LoadedSurface);
		if (!NewSDLTexture) {
			Debug::Log(EMessageType::ERR, "Texture", "Load", __TIMESTAMP__, __FILE__, __LINE__, "Failed to create Texture from " + FileName + ". SDL Error: " + SDL_GetError());
		}
		else {
